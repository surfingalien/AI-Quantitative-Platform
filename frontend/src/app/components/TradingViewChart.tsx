...
function generateMockCandles(ticker: string, count = 100): CandlestickData<Time>[] {
...
export default function TradingViewChart({ ticker }: { ticker: string }) {
  ...
  useEffect(() => {
    ...
    const chart = createChart(containerRef.current, {
      layout: { background: { type: ColorType.Solid, color: '#0a0c10' }, textColor: '#64748b', fontFamily: "'JetBrains Mono', monospace", fontSize: 11 },
      width: containerRef.current.clientWidth, height: 280,
      grid: { vertLines: { color: '#1e222d' }, horzLines: { color: '#1e222d' } },
      crosshair: { vertLine: { color: '#22d3a840', labelBackgroundColor: '#22d3a8' }, horzLine: { color: '#22d3a840', labelBackgroundColor: '#22d3a8' } },
      rightPriceScale: { borderColor: '#1e222d' },
      timeScale: { borderColor: '#1e222d', barSpacing: 7 },
    });
    chartRef.current = chart;
    const series = chart.addCandlestickSeries({ upColor: '#22d3a8', downColor: '#f43f5e', borderVisible: false, wickUpColor: '#22d3a8', wickDownColor: '#f43f5e' });
  ...
  return (
    <div>
      ...
      <p className="mt-1.5 text-[10px] text-[#64748b] font-mono">* Mock OHLCV — wire up price API to replace</p>
    </div>
  );
}
